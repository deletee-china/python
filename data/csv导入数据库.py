...
try:
    conn = pymysql.connect(host='localhost', user='root', password='lihang0422', db='mysql_zuoye', charset='utf8')
    cur = conn.cursor()
    print('数据库连接成功！')
    print(' ')
except:
...
for file in files:
    if file.split('.')[-1] in ['csv']:  #判断文件是不是csv文件，file.split('.')[-1]获取‘.’后的字符串
        ...
        f = pd.read_csv(file)  #用pandas读取文件，得到pandas框架格式的数据
